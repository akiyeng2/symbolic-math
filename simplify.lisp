...
(defsimple (sin) () (a)
  ((equalp a 0) 0)
  ((and (equalp a 'pi) (equalp a pi) 0))
  ((and (operator-memberp a '*) (member-if #'numberp a)
	(or (member pi a) (member 'pi a) (member 'π a)))
   (block eh
     (do-infix (k j) (p q)
       (when (or (eql (nth k a) 'pi) (eql (nth k a) 'π))
	 (return-from eh
	   (cond
	     ((eql (mod (* 12 (nth p a)) 24) 2) 1/2)
	     ((eql (mod (* 12 (nth p a)) 24) 3)
	      '((2 ^ 1/2) / 2))
	     ((eql (mod (* 12 (nth p a)) 24) 4)
	      '((3 ^ 1/2) / 2))
	     ((eql (mod (* 12 (nth p a)) 24) 6) 1)
	     ((eql (mod (* 12 (nth p a)) 24) 8)
	      '((3 ^ 1/2) / 2))
	     ((eql (mod (* 12 (nth p a)) 24) 9)
	      '((2 ^ 1/2) / 2))
	     ((eql (mod (* 12 (nth p a)) 24) 10) 1/2)
	     ((eql (mod (* 12 (nth p a)) 24) 12) 0)
	     ((eql (mod (* 12 (nth p a)) 24) 14) -1/2)
	     ((eql (mod (* 12 (nth p a)) 24) 15)
	      '(- ((2 ^ 1/2) / 2)))
	     ((eql (mod (* 12 (nth p a)) 24) 16)
	      '(- ((3 ^ 1/2) / 2)))
	     ((eql (mod (* 12 (nth p a)) 24) 18) -1)
	     ((eql (mod (* 12 (nth p a)) 24) 20)
	      '(- ((3 ^ 1/2) / 2)))
	     ((eql (mod (* 12 (nth p a)) 24) 21)
	      '(- ((2 ^ 1/2) / 2)))
	     ((eql (mod (* 12 (nth p a)) 24) 22) -1/2)
	     ((eql (mod (* 12 (nth p a)) 24) 0) 0)
	     (t `(sin ,a)))))))))
